...
exports.getAllEnrollUsers = async (req, res) => {
    try {
        const enrollData = await db.collection("participant").get()
...
exports.exportEnrollUsers = async (req, res) => {
    try {
        const courseId  = req.params.courseId
        const courseRef = db.doc(`courses/${courseId}`)
        ...
        const worksheet = workbook.addWorksheet("Participant Data")
        const path      = "./public/export"
        
        worksheet.columns = [
            { header: "User ID", key: "userId", width: 10 },
            { header: "Participant ID", key: "participantId", width: 10 }, 
        ...
        await workbook.xlsx.writeFile(`${path}/${courseId}.xlsx`)

        res.status(200).json({
            code: "OK",
            message: "Success retrieve all data.",
            path: `${path}/${courseId}.xlsx`
        })
    } catch (error) {
        console.log(new Error(error.messages ? error.messages : error.message))
        res.status(`${error.status ? error.status : 500}`).json({
